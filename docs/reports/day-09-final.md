...
- 开发周期：9 天。
- 课题名称：浏览器通过 Go 回环服务调用本地打印能力（课题三）。
- 成员与分工：独立完成；需求梳理、范围决策、架构、编码、测试、演示验证、文档与提交整理均由本人承担。
...
对照本组必做范围（两类业务 PDF、FIFO、任务状态与失败重试、本地 API 与页面、Windows/Linux Adapter、可复现文档）逐条给出结论；每行附一句证据说明。
...
前提：Go 1.23+ 与 Chrome/Chromium 131+。demo 模式的打印机为“Mock Printer（不执行实体打印）”，全程不访问系统打印队列。

1. 项目根目录执行 `.\scripts\run-windows.ps1 -Mode demo -GoCachePath '.cache\go-build'`（Linux 用 `./scripts/run-linux.sh --mode demo --go-cache .cache/go-build`）。预期：终端输出回环监听地址。
2. 浏览器打开该地址。预期：页面确认 health，打印机一栏显示 Mock Printer。
3. 在气球表单输入队伍名、题号和通过时间并创建。预期：任务列表出现新任务，数秒内变为 `succeeded`（attempts=1）。
4. 点击“打开 PDF 预览”。预期：80 x 120 mm 窄纸小票，中文与业务字段正确。
5. 提交含中文注释的源码任务（可用 `testdata/source_cpp.json` 内容）。预期：多页 A4，高亮、行号、页眉页码齐全。
6. 对 failed 任务点击重试。预期：清错误重新排队，attempts 保留。
7. 启动终端 Ctrl+C。预期：服务退出，端口可重新绑定。

platform 模式演示（已补验）：在已确认安全的隔离队列上执行同样的创建与预览步骤，并额外展示打印机枚举、隔离输出文件与队列作业记录；每段演示明确说明“队列接受不等于物理出纸”。
...
九天里沟通方式有明确改进：从早期的“帮我检查/补充”逐步收敛为“现象 / 期望 / 已尝试”三段式排错提问和“限定范围 + 验收标准”的实现提问，得到空话的比例明显下降。

如果还能再有若干天完善，优先改进三件事：其一，把 platform 队列证据做成显式 opt-in 的自动化测试（当前依赖人工补验流程）；其二，为队列引入数据库与平台作业号持久化，收窄 at-least-once 崩溃窗口的人工核对成本；其三，补充多模板与打印目标偏好设置，贴近竞赛现场的实际分工。
...
按代码与自动验证范围，核心主路径已经完成。结课时人工验收尚缺的平台队列证据已于 2026-08-21 在双平台隔离安全队列上补验完成（含 CUPS request id 与连续录屏）；原计划的真人 README 冷启动验收于 2026-08-22 取消，以实现者干净副本复现代替。结课时列出的补验计划与最终结果对照如下：
...
| Linux/CUPS platform 补验（2026-08-21） | `docs/reports/assets/linux-platform-evidence-2026-08-21.md`（结构化证据，含 CUPS request id 与录屏记录）、`linux-iso-output-2026-08-21.pdf`（隔离队列产出副本）、`linux-platform-queue-2026-08-21.mp4`（连续录屏，不入版本控制） |
