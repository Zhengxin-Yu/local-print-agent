...
| 现象/错误码 | 原因与处理 |
| --- | --- |
| `RENDERER_NOT_FOUND` | Chrome/Chromium 路径不存在。给脚本传入仓库根目录下的正确相对路径。 |
...
- Windows 系统队列、Linux/CUPS 实跑和双平台录屏证据已于 2026-08-21 在隔离安全队列环境补齐（见 `docs/reports/assets/`），证明系统队列接受与隔离输出；物理出纸未验证。
